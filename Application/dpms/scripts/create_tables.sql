...
CREATE TABLE F21_S001_22_payment_plan(
	...
	CONSTRAINT plan_status_ck CHECK (status = 'patient-payment-pending' OR status = 'payment-done' OR status = 'insurance-payment-pending' OR status = 'pending-insurance-approval' OR status = 'insurance-denied')
);
